...
void CGrannyModelInstance::__CreateMeshMatrices()
{
	...
	if (m_pModel->GetMeshCount() <= 0) // ¸Ţ˝¬°ˇ ľř´Â (Ä«¸Ţ¶ó °°Ŕş) ¸đµ¨µµ °ŁČ¤ ŔÖ´Ů..
		return;
...
{
	// NOTE : GrannyGetWorldPose4x4´Â ˝şÄÉŔĎ °ŞµîŔĚ Ŕß¸řłŞżĂ Ľö ŔÖŔ˝.. ±×·ˇ´Ď°ˇ ĽÓµµ¸¦ Ŕ§ÇŘ
	//        GrannyGetWorldPose4x4żˇ ¸đµç matrix żřĽŇ¸¦ Á¦ °ŞŔ¸·Î łÖÁö ľĘŔ˝
	return GrannyGetWorldPoseComposite4x4(__GetWorldPosePtr(), iBone);
}

void CGrannyModelInstance::ReloadTexture()
{
	assert("ÇöŔç »çżëÇĎÁö ľĘŔ˝ - CGrannyModelInstance::ReloadTexture()");
/*
	assert(m_pModel != NULL);
