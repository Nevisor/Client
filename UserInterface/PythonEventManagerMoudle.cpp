...
PyObject * eventGetTotalLineCount(PyObject * poSelf, PyObject * poArgs) //NEW
{
	int iIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iIndex))
		return Py_BuildException();
...
PyObject * eventSetVisibleLineCount(PyObject * poSelf, PyObject * poArgs) //NEW
{
	int iIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iIndex))
		return Py_BuildException();
...
PyObject * eventSetFontColor(PyObject * poSelf, PyObject * poArgs) //NEW
{
	int iIndex;
	float fr, fg, fb;
	...
	CPythonEventManager::Instance().SetVisibleFontColor(iIndex, fr, fg, fb);
	return Py_BuildNone();
}
...
void initEvent()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "GetTotalLineCount",			eventGetTotalLineCount,				METH_VARARGS }, //NEW

		{ "SetVisibleLineCount",		eventSetVisibleLineCount,			METH_VARARGS }, //NEW
		{ "SetFontColor",				eventSetFontColor,					METH_VARARGS }, //NEW
