...
bool RunMainScript(CPythonLauncher& pyLauncher, const char* lpCmdLine)
{
	...
	// RegisterCommandLine
	{
		...
		{
			char buf[MAX_PATH];	//TODO ľĆ·ˇ ÇÔĽö string ÇüĹÂ·Î ĽöÁ¤
...
bool Main(HINSTANCE hInstance, LPSTR lpCmdLine)
{
	...
	bool ret=false;
	{
		...
		{
			ret=RunMainScript(pyLauncher, lpCmdLine);	//°ÔŔÓ ˝ÇÇŕÁßżŁ ÇÔĽö°ˇ łˇłŞÁö ľĘ´Â´Ů.
		}
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	bool bQuit = false;
	bool bAuthKeyChecked = false;	//OpenID ąöŔüżˇĽ­ ŔÎÁőĹ°°ˇ µéľîżÔ´ÂÁö ľË±â Ŕ§ÇŃ ŔÎŔÚ.
	int nArgc = 0;
	...
	for( int i=0; i < nArgc; i++ ) {
		if(szArgv[i] == 0)
			continue;
		if (__IsLocaleVersion(szArgv[i])) // #0000829: [M2EU] ąöŔü ĆÄŔĎŔĚ Ç×»ó »ý±âÁö ľĘµµ·Ď ĽöÁ¤ 
		{
		...
		{
			// locale ĽłÁ¤żŁ ŔÎŔÚ°ˇ µÎ °ł ´ő ÇĘżäÇÔ (·ÎÄÉŔĎ ¸íÄŞ, µĄŔĚĹÍ °ć·Î)
			if (nArgc <= i + 2)
			{
	...
	// żÉĽÇŔĚ ľřŔ¸¸é şńÁ¤»ó ˝ÇÇŕŔ¸·Î °ŁÁÖ, ÇÁ·Î±×·Ą Áľ·á
