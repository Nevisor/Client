...
{
	rEventSet.iTotalLineCount = 0; //NEW
...
int CPythonEventManager::RegisterEventSet(const char * c_szFileName)
{
	...
	if (!CEterPackManager::Instance().Get(File2, c_szFileName, &pData)) //NEW
		return false;

	CMemoryTextFileLoader textFileLoader;
	textFileLoader.Bind(File2.Size(), pData);

	CTokenVector TokenVector;

	for (DWORD i = 0; i < textFileLoader.GetLineCount(); ++i) //NEW
	{
		++iCount;
	}

	pEventSet->iTotalLineCount = iCount - 1; //skip [DELAY=i]
...
void CPythonEventManager::SetVisibleLineCount(int iIndex, int iLineCount) //NEW
{
	if (!CheckEventSetIndex(iIndex))
		return;
...
int CPythonEventManager::GetTotalLineCount(int iIndex) //NEW
{
	if (!CheckEventSetIndex(iIndex))
		return 0;
...
void CPythonEventManager::SetVisibleFontColor(int iIndex, float fr, float fg, float fb) //NEW
{
	if (!CheckEventSetIndex(iIndex))
		return;
	...
	pEventSet->CurrentColor.r = (float)fr;
	pEventSet->CurrentColor.g = (float)fg;
	pEventSet->CurrentColor.b = (float)fb;
	pEventSet->CurrentColor.a = 1.0f;
}
