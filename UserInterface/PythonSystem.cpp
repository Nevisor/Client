...
{
	return m_Config.bDiceChatShow;
}

void CPythonSystem::SetDiceChatShow(int iFlag)
{
	m_Config.bDiceChatShow = iFlag == 1 ? true : false;
}
...
bool CPythonSystem::LoadConfig()
{
	...
	while (fgets(buf, 256, fp))
	{
		...
		{
			m_Config.bDiceChatShow = atoi(value) == 1 ? true : false;
			CPythonChat::Instance().SetChatFilter(0, CHAT_TYPE_DICE_INFO, atoi(value) == 1 ? true : false);
		}
	}
...
bool CPythonSystem::SaveConfig()
{
	...
	if (m_Config.bDiceChatShow == 0)
		fprintf(fp, "CHAT_FILTER_DICE		%d\n", m_Config.bDiceChatShow);
